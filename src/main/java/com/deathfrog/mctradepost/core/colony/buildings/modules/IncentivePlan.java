...
public final class IncentivePlan
{
    ...
    /** @return whether this plan should attempt another daily purchase */
     ...
     */
    public long dailyCost(final int price, final ToIntFunction<Skill> currentLevel)
    {
    ...
    /**
     * Expires the previous adjustments and purchases at most one package for the current day.
     ...
     */
    public boolean processDay(final int day, final int price, final ToIntFunction<Skill> level,
        final BiConsumer<Skill, Integer> adjust, final LongPredicate withdraw)
    {
        if (day <= lastProcessedDay || day < startsOnDay) return false;
