...
public class CoinEntity extends ItemEntity implements ItemSupplier {
    ...
    public CoinEntity(EntityType<CoinEntity> type, Level level) {
        super(type, level);
        this.setUnlimitedLifetime(); // if you want it to persist
        this.coinStack = new ItemStack(MCTradePostMod.MCTP_COIN_ITEM.get());
    }
    ...
    /**
     * Replaces dropped coins with our custom CoinEntity, which checks if
     * it landed in a wishing well and handles the effect accordingly.
     * 
    ...
    public static void onItemDrop(EntityJoinLevelEvent event) {
        ...
        if (itemEntity.getClass().getSimpleName().equals("CoinEntity")) return;
