...
public class PetPanda extends Panda implements ITradePostPet, IHerdingPet, IManagedAnimal<PetPanda>
{
    private static final double MOVEMENT_SPEED = 0.25D;
    private static final double LAZY_MOVEMENT_SPEED = 0.20D;

    public static final Logger LOGGER = LogUtils.getLogger();
    @SuppressWarnings("null")
    public static final @Nonnull EntityDataAccessor<Integer> DATA_COLONY_ID = SynchedEntityData.defineId(PetPanda.class, EntityDataSerializers.INT);
    @SuppressWarnings("null")
    public static final @Nonnull EntityDataAccessor<Integer> DATA_MANAGED_ANIMAL_ID = SynchedEntityData.defineId(PetPanda.class, EntityDataSerializers.INT);

    public int logCooldown = 0;

    protected PetData<PetPanda> petData = null;
    ...
    @SuppressWarnings("null")
    public PetPanda(EntityType<? extends Panda> entityType, Level level)
    {
        super(entityType, level);
        petData = new PetData<>(this);
        setMainGene(Panda.Gene.getRandom(this.random));
        setHiddenGene(Panda.Gene.getRandom(this.random));
        setAttributes();
        registerGoals();
    }

    /**
     * Applies vanilla gene attributes while keeping lazy trained pandas fast enough
     * to perform colony jobs. Non-lazy pandas retain the registered 0.25 base speed.
     */
    @SuppressWarnings("null")
    @Override
    public void setAttributes()
    {
        ...
        this.getAttribute(Attributes.MOVEMENT_SPEED)
            .setBaseValue(this.isLazy() ? LAZY_MOVEMENT_SPEED : MOVEMENT_SPEED);
    }
    ...
    public void remove(@Nonnull RemovalReason reason)
    {
        ...
        switch (reason) {
            ...
            case UNLOADED_TO_CHUNK, CHANGED_DIMENSION -> {
                // benign; donâ€™t clear persistent state
            }
            default -> {
                // benign; donâ€™t clear persistent state
            }
    ...
    public PetData<PetPanda> getPetData()
    {
        return this.petData;
    }
